var map, featureList, BaresSearch = [], DepositosSearch = [];
...
function syncSidebar() {
  ...
  Bares.eachLayer(function (layer) {
    if (map.hasLayer(BaresLayer)) {
  ...
  Depositos.eachLayer(function (layer) {
    if (map.hasLayer(DepositosLayer)) {
      if (map.getBounds().contains(layer.getLatLng())) {
...
var BaresLayer = L.geoJson(null);
var Bares = L.geoJson(null, {
...
$.getJSON("data/DOITT_Bares_01_13SEPT2010.geojson", function (data) {
  Bares.addData(data);
  map.addLayer(BaresLayer);
});

/* Empty layer placeholder to add to layer control for listening when to add/remove hoteles to markerClusters layer */
var DepositosLayer = L.geoJson(null);
var Depositos = L.geoJson(null, {
  pointToLayer: function (feature, latlng) {
    return L.marker(latlng, {
      icon: L.icon({
...
$.getJSON("data/DOITT_Depositos_01_13SEPT2010.geojson", function (data) {
  Depositos.addData(data);
    map.addLayer(DepositosLayer);
});
...
map.on("overlayadd", function(e) {
  if (e.layer === BaresLayer) {
    markerClusters.addLayer(Bares);
    syncSidebar();
  }
  if (e.layer === DepositosLayer) {
    markerClusters.addLayer(Depositos);
    syncSidebar();
  }
});
...
map.on("overlayremove", function(e) {
  if (e.layer === BaresLayer) {
    markerClusters.removeLayer(Bares);
    syncSidebar();
  }
  if (e.layer === DepositosLayer) {
    markerClusters.removeLayer(Depositos);
    syncSidebar();
  }
});
...
var groupedOverlays = {
  "Puntos de interes": {
    "<img src='assets/img/0.png' width='24' height='28'>&nbsp;Bares": BaresLayer,
    "<img src='assets/img/013.png' width='24' height='28'>&nbsp;Depositos": DepositosLayer
  },
...
$(document).one("ajaxStop", function () {
  ...
  var DepositosBH = new Bloodhound({
    name: "Depositos",
    datumTokenizer: function (d) {
      return Bloodhound.tokenizers.whitespace(d.name);
    },
    queryTokenizer: Bloodhound.tokenizers.whitespace,
    local: DepositosSearch,
    limit: 10
  });
  ...
  $("#searchbox").typeahead({
    ...
    name: "Bares",
    displayKey: "name",
    source: BaresBH.ttAdapter(),
    templates: {
      header: "<h4 class='typeahead-header'><img src='assets/img/0.png' width='24' height='28'>&nbsp;Bares</h4>",
      suggestion: Handlebars.compile(["{{name}}<br>&nbsp;<small>{{address}}</small>"].join(""))
    }
  }, {
    name: "Depositos",
    displayKey: "name",
    source: DepositosBH.ttAdapter(),
    templates: {
      header: "<h4 class='typeahead-header'><img src='assets/img/013.png' width='24' height='28'>&nbsp;Depositos</h4>",
      suggestion: Handlebars.compile(["{{name}}<br>&nbsp;<small>{{address}}</small>"].join(""))
    }
  }, {
    name: "GeoNames",
